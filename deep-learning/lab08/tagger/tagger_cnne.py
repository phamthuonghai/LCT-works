    ...
    def construct(self, args, num_words, num_chars, num_tags):
        with self.session.graph.as_default():
            ...
            for kernel_size in range(2, args.cnne_max+1):
                cnne_layer = tf.layers.conv1d(char_embedded, args.cnne_filters, kernel_size,
                                              strides=1, padding='valid', activation=None)
                cnne_layer = tf.reduce_max(cnne_layer, axis=1)
                cnne_features.append(cnne_layer)
            ...
            cnne_feature = tf.concat(cnne_features, axis=1)
            ...
            cnne = tf.nn.embedding_lookup(cnne_feature, self.charseq_ids)

            # Concatenate the word embeddings (computed above) and the CNNE (in this order).
            embedded = tf.concat([word_embedded, cnne], axis=2)
