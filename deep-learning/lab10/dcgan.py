...
class Network:
    ...
    def construct(self, args):
        ...
        with self.session.graph.as_default():
            ...
            def generator(z):
                ...
                features = tf.layers.dense(z, 1024, activation=None, use_bias=False)
                features = tf.layers.batch_normalization(features, training=True)
                features = tf.nn.relu(features)

                features = tf.layers.dense(features, 7 * 7 * 64, activation=None, use_bias=False)
                features = tf.layers.batch_normalization(features, training=True)
                features = tf.nn.relu(features)

                features = tf.reshape(features, [-1, 7, 7, 64])

                features = tf.layers.conv2d_transpose(features, 32, 5, 2, padding='same', activation=None,
                                                      use_bias=False)
                features = tf.layers.batch_normalization(features, training=True)
                features = tf.nn.relu(features)

                features = tf.layers.conv2d_transpose(features, 1, 5, 2, padding='same', activation=tf.nn.sigmoid)
            ...
            def discriminator(image):
                ...
                features = tf.layers.conv2d(image, 32, 5, padding='same', activation=None, use_bias=False)
                features = tf.layers.batch_normalization(features, training=True)
                features = tf.nn.relu(features)

                features = tf.layers.max_pooling2d(features, 2, 2)

                features = tf.layers.conv2d(features, 64, 5, padding='same', activation=None, use_bias=False)
                features = tf.layers.batch_normalization(features, training=True)
                features = tf.nn.relu(features)
                ...
                features = tf.layers.dense(features, 1024, activation=None, use_bias=False)
                features = tf.layers.batch_normalization(features, training=True)
                features = tf.nn.relu(features)

                features = tf.layers.dense(features, 1, activation=None)
            ...
            self.discriminator_loss = tf.losses.sigmoid_cross_entropy(1.0, discriminator_logit_real) + \
                tf.losses.sigmoid_cross_entropy(0.0, discriminator_logit_fake)

            # Define `self.generator_loss` as a sigmoid cross entropy
            # loss with gold labels of ones (1.0) and discriminator_logit_fake.
            self.generator_loss = tf.losses.sigmoid_cross_entropy(1.0, discriminator_logit_fake)
            ...
            self.generator_training = tf.train.AdamOptimizer().minimize(
                self.generator_loss, var_list=tf.global_variables("generator"), global_step=global_step)
    ...
    def train(self, images):
        ...
        dis_loss = self.session.run([self.discriminator_training, self.discriminator_summary, self.discriminator_loss],
                                    feed_dict={
                                        self.images: images,
                                        self.z: self.sample_z(self.batch_size)
                                    })[-1]
        ...
        gen_loss = self.session.run([self.generator_training, self.generator_summary, self.generator_loss],
                                    feed_dict={
                                        self.z: self.sample_z(self.batch_size)
                                    })[-1]
