...
class Network:
    ...
    def construct(self, args):
        with self.session.graph.as_default():
            ...
            sequence = tf.reshape(self.sequence, [-1, 1, 1])
            ...
            rnn_cell = cell_type[args.rnn_cell](num_units=args.rnn_cell_dim)

            # Create zero state using rnn_cell.zero_state call. Use batch size 1.
            state = rnn_cell.zero_state(1, dtype=tf.float32)
            predictions, loss = [], 0
            dense = tf.layers.Dense(1)  # Create a dense layer object using tf.layers.Dense, with 1 output unit.
            ...
            for i in range(self.TEST):
                ...
                output, state = rnn_cell(predictions[self.TRAIN + i - 1], state)
                predictions.append(dense(output))

            # Generate `self.predictions` tensor (instead of Python list), use `tf.stack`.
            self.predictions = tf.squeeze(tf.stack(predictions))
    ...
    def prediction_summary(self, gold, predictions):
        ...
        pred_loss, _ = self.session.run([self.prediction_loss, self.summaries["prediction"]],
                                     {self.predictions: predictions,
                                      self.prediction_gold: gold[self.TRAIN:],
...
if __name__ == "__main__":
    ...
    for epoch in range(args.epochs):
        ...
        pred_loss = network.prediction_summary(data, network.predict(data[:Network.TRAIN]))
