...
class Network:
    ...
    def construct(self, args, batches_per_epoch):
        with self.session.graph.as_default():
            ...
            # Create `optimizer` according to arguments ("SGD", "SGD" with momentum, or "Adam"),
            ...
            with summary_writer.as_default(), tf.contrib.summary.record_summaries_every_n_global_steps(100):
                self.summaries["train"] = [tf.contrib.summary.scalar("train/loss", loss),
                                           ...
                                           tf.contrib.summary.scalar("train/learning_rate", lr)]
            with summary_writer.as_default(), tf.contrib.summary.always_record_summaries():
                for dataset in ["dev", "test"]:
                    self.summaries[dataset] = [tf.contrib.summary.scalar(dataset + "/accuracy", self.accuracy),
                                               tf.contrib.summary.scalar(dataset + "/learning_rate", lr)]
...
if __name__ == "__main__":
    ...
    test_result = network.evaluate("test", mnist.test.images, mnist.test.labels)

    # Compute accuracy on the test set and print it as percentage rounded
    # to two decimal places.
    print("{:.2f}".format(100 * test_result[0]))
